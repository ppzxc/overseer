...
def test_bao_ctrl_002_ssh_ca_mount_and_key(http_session, openbao_url):
    ...
    try:
        resp = http_session.get(f"{openbao_url}/v1/ssh-client-signer/public_key", timeout=3)
        ...
        pytest.skip(f"OpenBao is not running at {openbao_url} (live stack required for runtime CA test)")
...
def test_bao_ctrl_003_signing_role(http_session, openbao_url, openbao_token):
    ...
    # 1. 서명 역할(Role) 확인
    try:
        role_resp = http_session.get(f"{openbao_url}/v1/ssh-client-signer/roles/infra-admin-role", headers=headers, timeout=3)
    except (requests.exceptions.ConnectionError, requests.exceptions.Timeout):
        pytest.skip(f"OpenBao is not running at {openbao_url} (live stack required for signing test)")
