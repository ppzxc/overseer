...
본 문서는 **Overseer**의 중앙 컨트롤 플레인(OpenBao, Boundary, Postgres, Prometheus)과 온프레미스 노드 프로비저닝(Ansible)이 유기적으로 연결되어 동작하는지 검증하는 **E2E 시스템 통합 테스트 스위트(`tests/`)**의 구조와 실행 가이드를 정의합니다.
...
│  [test_01_control_plane.py]     ───> Postgres, OpenBao, Boundary, Prometheus│
...
| `test_01_control_plane.py` | PostgreSQL(5432), OpenBao(8200), Boundary(9200), Prometheus(9090) 포트 및 HTTP 헬스체크 |
...
# 특정 테스트 모듈만 실행
pytest tests/test_02_openbao_ssh_ca.py -v
```
